    ...
    public SimpleWeightedGraph<String, MyWeightedEdge> Parse(ArrayList<String> f) throws IOException {
        ...
        for(String path : f){
            ...
            for (CtType type : model.getAllTypes()){
                ...
                for(Object method : type.getMethods()){
                    ...
                    p.filterChildren(new TypeFilter(CtInvocation.class)).forEach(inv->{
                        ...
                        if(invocation.getExecutable().getDeclaringType() != null) {
                            ...
                            if (!map.containsKey(type.getSimpleName()+":"+ invocation.getExecutable().getDeclaringType().getSimpleName()) && !map.containsKey(invocation.getExecutable().getDeclaringType().getSimpleName()+":"+ type.getSimpleName().toString())) {
                                //Si elle y est pas on met le lien dans la hashmap sous forme A:B
                                map.put(type.getSimpleName().toString()+":"+ invocation.getExecutable().getDeclaringType().getSimpleName(),1);


                            } else {
                                //Si le lien existe on incrémente le nombre d'appel invoqué (On vérifie si c'est le lien A:B ou B:A)
                                if(map.containsKey(type.getSimpleName().toString()+":"+ invocation.getExecutable().getDeclaringType().getSimpleName())){
                                    //on incrémente le nombre d'appel de la classe A vers B
                                    map.put(type.getSimpleName().toString()+":"+ invocation.getExecutable().getDeclaringType().getSimpleName(), map.get(type.getSimpleName().toString()+":"+ invocation.getExecutable().getDeclaringType().getSimpleName())+1);
                                }else{
                                    //on incrémente le nombre d'appel de la classe A vers B
                                    map.put(invocation.getExecutable().getDeclaringType().getSimpleName()+":"+type.getSimpleName().toString(), map.get(invocation.getExecutable().getDeclaringType().getSimpleName()+":"+type.getSimpleName().toString() )+1);
                                }
                            }
                        }
                    });
                ...
                nbinvocation.put(type.getSimpleName(),nbappel.get());
            }
